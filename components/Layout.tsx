...
import { UserRole, Idea, AppView } from '../types';
import { storageService } from '../services/storageService';
import OCDAIAgent from './OCDAIAgent';
...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const NavItem: React.FC<{ label: string; active: boolean; onClick: () => void; icon?: React.ReactNode }> = ({ label, active, onClick, icon }) => (
    ...
    >
      ...
      {active && (
        <span className="absolute -bottom-1 w-1 h-1 bg-blue-600 dark:bg-blue-400 rounded-full animate-pulse"></span>
      )}
    </button>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col transition-colors duration-300 overflow-x-hidden">
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-3 w-52 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-fade-in py-1">
                    ...
                    >
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-300">User Management</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-rose-600">Logout Session</span>
                    </button>
                  </div>
                )}
              </div>
      ...
      {/* OCD AI Agent Floating Component */}
      <OCDAIAgent />
